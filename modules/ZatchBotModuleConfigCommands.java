...
public class ZatchBotModuleConfigCommands extends ZatchBotModule_Config{
	...
	public void loadConfig(String chan) throws Exception{
		...
		if(!configVersion.startsWith("Config Version")){
		    ...
		    ZatchBotModuleConfigStartup.OpHostnameUsed = Boolean.parseBoolean(saveFile.readLine()); //10th line
		    saveFile.readLine(); //11th line
		    ZatchBotModuleConfigStartup.OpNickUsed = Boolean.parseBoolean(saveFile.readLine()); //12th line
		    saveFile.readLine(); //13th line
		    ZatchBotModuleConfigStartup.BotNick = saveFile.readLine(); //14th line
		    saveFile.readLine(); //15th line
		    ZatchBotModuleConfigStartup.Master = saveFile.readLine(); //16th line
		    saveFile.readLine(); //17th line
		    ZatchBotModuleConfigStartup.OpNick = saveFile.readLine(); //18th line
		    saveFile.readLine(); //19th line
		    ZatchBotModuleConfigStartup.OpHostname = saveFile.readLine(); //10h line
		    saveFile.readLine(); //21th line 
		    boolean logsMode = Boolean.parseBoolean(saveFile.readLine()); //22st line
		    saveFile.readLine(); //23nd Line
		    if(logsMode == true){
		    	new ZatchBotLogging().setLoggingMode(true);
		    	logging.setLoggingLocation(saveFile.readLine()); //24rd line
		    }
		    else{
                new ZatchBotLogging().setLoggingMode(false);
